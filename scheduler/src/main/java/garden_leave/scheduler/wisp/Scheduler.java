...
import garden_leave.scheduler.wisp.stats.SchedulerStats;
import garden_leave.scheduler.wisp.stats.ThreadPoolStats;
...
public final class Scheduler {
  ...
  public Job schedule(String nullableName, Runnable runnable, Schedule when) {
    return schedule(nullableName, runnable, when, 0);
  }

  public Job schedule(
      String nullableName, Runnable runnable, Schedule when, int maxExecutionCount) {
    ...
    Job job = prepareJob(name, runnable, when, maxExecutionCount);
  ...
  public CompletionStage<Job> cancel(String jobName) {
    ...
    synchronized (this) {
      ...
      if (jobStatus == JobStatus.RUNNING
        ...
        throw new IllegalStateException("Cannot find the job " + job + " in " + nextExecutionsOrder
  ...
  public SchedulerStats stats() {
    ...
    return SchedulerStats.of(ThreadPoolStats.of(threadPoolExecutor.getCorePoolSize(),
        threadPoolExecutor.getMaximumPoolSize(), activeThreads,
        threadPoolExecutor.getPoolSize() - activeThreads, threadPoolExecutor.getLargestPoolSize()));
  }

  // internal

  private Job prepareJob(String name, Runnable runnable, Schedule when, int maxExecutionCount) {
    ...
    synchronized (indexedJobsByName) {
      ...
      Job job = new Job(JobStatus.DONE, 0L, lastJob != null ? lastJob.executionsCount() : 0,
          lastJob != null ? lastJob.maxExecutionsCount() : maxExecutionCount,
          lastJob != null ? lastJob.lastExecutionStartedTimeInMillis() : null,
          lastJob != null ? lastJob.lastExecutionEndedTimeInMillis() : null, name, when, runnable);
  ...
  private synchronized void scheduleNextExecution(Job job) {
    ...
    long currentTimeInMillis = timeProvider.currentTime();
    try {
      job.nextExecutionTimeInMillis(job.schedule().nextExecutionInMillis(
          currentTimeInMillis, job.executionsCount(), job.lastExecutionEndedTimeInMillis()));
    } catch (Throwable t) {
      logger.error("An exception was raised during the job next execution time calculation,"
              + " therefore the job '{}' will not be executed again.",
          job.name(), t);
      job.nextExecutionTimeInMillis(Schedule.WILL_NOT_BE_EXECUTED_AGAIN);
    }

    if (job.maxExecutionsCount() != 0 && job.executionsCount() >= job.maxExecutionsCount()) {
      logger.info(
          "Job '{}' has reached max executions count {}", job.name(), job.maxExecutionsCount());
      job.nextExecutionTimeInMillis(Schedule.WILL_NOT_BE_EXECUTED_AGAIN);
    }
    ...
    if (job.nextExecutionTimeInMillis() >= currentTimeInMillis) {
      ...
      nextExecutionsOrder.sort(Comparator.comparing(Job::nextExecutionTimeInMillis));
      ...
      logger.info(
          ...
          job.name(), Instant.ofEpochMilli(job.nextExecutionTimeInMillis()));
      job.status(JobStatus.DONE);
    ...
    while (!shuttingDown) {
      ...
      synchronized (this) {
        if (nextExecutionsOrder.size() > 0) {
          timeBeforeNextExecution =
              nextExecutionsOrder.get(0).nextExecutionTimeInMillis() - timeProvider.currentTime();
        }
      }
  ...
  private void runJob(Job jobToRun) {
    ...
    try {
      ...
      // Only increment executions count on successful run
    ...
    if (logger.isDebugEnabled()) {
      logger.debug("Job '{}' executed in {}ms", jobToRun.name(),
