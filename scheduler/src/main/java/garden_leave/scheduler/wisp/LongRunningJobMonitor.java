...
public class LongRunningJobMonitor implements Runnable {
   ...
   * @param detectionThreshold The threshold after which a message will be logged if the job takes
   *     longer to execute
   * @param timeProvider The time provider used to calculate long running jobs
   */
  public LongRunningJobMonitor(
      Scheduler scheduler, Duration detectionThreshold, TimeProvider timeProvider) {
  ...
  boolean detectLongRunningJob(long currentTime, Job job) {
    if (job.status() == JobStatus.RUNNING && !longRunningJobs.containsKey(job)) {
      ...
      if (jobStartedtimeInMillis != null && threadRunningJob != null
        ...
        logger.warn(
            ...
            job.name(), currentTime - jobStartedtimeInMillis, detectionThresholdInMillis,
            Stream.of(threadRunningJob.getStackTrace())
                .map(StackTraceElement::toString)
                .collect(Collectors.joining("\n  ")));

        longRunningJobs.put(
            job, new LongRunningJobInfo(jobStartedtimeInMillis, jobExecutionsCount));
  ...
  Long cleanUpLongJobIfItHasFinishedExecuting(long currentTime, Job job) {
    if (longRunningJobs.containsKey(job)
      ...
      if (jobExecutionsCount == jobRunningInfo.executionsCount + 1) {
        ...
        logger.info("Job '{}' has finished executing after {}ms", job.name(), jobExecutionDuration);
      } else {
        jobExecutionDuration = currentTime - jobRunningInfo.jobStartedtimeInMillis;
        logger.info(
            "Job '{}' has finished executing after about {}ms", job.name(), jobExecutionDuration);
      }
