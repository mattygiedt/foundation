...
public class FixedTimeSchedule implements Schedule {
   ...
   */
  public FixedTimeSchedule(String every) {
   ...
   * Parse time in the form of "hh:mm" or "hh:mm:ss"
   */
  public FixedTimeSchedule(String every, ZoneId zoneId) {
    this(LocalTime.parse(every), zoneId);
  }

  public FixedTimeSchedule(LocalTime every) {
    this(every, ZoneId.systemDefault());
  }

  public FixedTimeSchedule(LocalTime every, ZoneId zoneId) {
  ...
  long durationUntilNextExecutionInMillis(
      long currentTimeInMillis, Long lastExecutionTimeInMillis) {
    ZonedDateTime currentDateTime = Instant.ofEpochMilli(currentTimeInMillis).atZone(zoneId);

    return currentDateTime.until(
        nextExecutionDateTime(currentDateTime,
            lastExecutionTimeInMillis != null && lastExecutionTimeInMillis == currentTimeInMillis),
        ChronoUnit.MILLIS);
  }

  private ZonedDateTime nextExecutionDateTime(
      ZonedDateTime currentDateTime, boolean nextExecutionShouldBeNextDay) {
    if (!nextExecutionShouldBeNextDay
        && currentDateTime.toLocalTime().compareTo(executionTime) <= 0) {
