package garden_leave.scheduler.cron;
 ...
 * used when one want to specify something for one of those two fields, but not the other. <P>
 ...
 * field means &quot;for monday, wednesday and friday&quot; <P>
 * '/' Used to specify increments. E.g. &quot;0/15&quot; in Seconds field means &quot;for seconds 0,
 * 15, 30, ad 45&quot;. And &quot;5/15&quot; in seconds field means &quot;for seconds 5, 20, 35, and
 * 50&quot;. If '*' s specified before '/' it is the same as saying it starts at 0. For every field
 * there's a list of values that can be turned on or off. For Seconds and Minutes these range from
 * 0-59. For Hours from 0 to 23, For Day-of-month it's 1 to 31, For Months 1 to 12. &quot;/&quot;
 * character helsp turn some of these values back on. Thus &quot;7/6&quot; in Months field specify
 * just Month 7. It doesn't turn on every 6 month following, since cron fields never roll over <P>
 ...
 * subtracted (5L in Day-of-month means 5 days before last day of Month: 26 jan, 23 feb, etc.) <P>
 ...
 * aren't 5 fridays in the month, then it won't match until the next month with 5 fridays. <P>
 ...
 * doesn't match until the constraints of each field are met. Overlap of intervals are not allowed.
 * That is: for Day-of-week field &quot;FRI-MON&quot; is invalid,but &quot;FRI-SUN,MON&quot; is
 * valid
 *
 */
public class CronExpression {
  enum CronFieldType {
    ...
    MONTH(1, 12,
      ...
      @Override
      ZonedDateTime setValue(ZonedDateTime dateTime, int value) {
        return dateTime.withMonth(value)
            .withDayOfMonth(1)
            .withHour(0)
            .withMinute(0)
            .withSecond(0)
  ...
  public CronExpression(final String expr, final boolean withSeconds) {
    ...
    if (parts.length != expectedParts) {
      throw new IllegalArgumentException(
          String.format("Invalid cron expression [%s], expected %s field, got %s", expr,
   ...
   * <p>
   * This is handled different than all other fields because there are two ways to describe the day
   * and it is easier to handle them together in the same method.
   *
  ...
  abstract static class BasicField {
    ...
    private void validateRange(FieldPart part) {
      if ((part.from != -1 && part.from < fieldType.from)
        ...
        throw new IllegalArgumentException(
            String.format("Invalid interval [%s-%s], must be %s<=_<=%s", part.from, part.to,
                fieldType.from, fieldType.to));
